...
static void *triangle_stage(void *a)
{
  ...
  while (triangle_creator_get_next_triangle(triangle_creator, &triangle))
  {
    ...
    if (triangle_is_sync(&triangle))
    {
      ...
      struct Fragment f_sync = fragment_sync();
      ...
      pthread_barrier_wait(&dispatch_barrier);
    }
#endif
  }
...
int main(int argc, char *argv[])
{
  ...
  pthread_barrier_init(&dispatch_barrier, NULL, 1 + nb_rasters + nb_shaders);
  ...
  // Suppresion
  pthread_barrier_destroy(&raster_barrier);
  pthread_barrier_destroy(&shader_barrier);
  pthread_barrier_destroy(&dispatch_barrier);
  sem_destroy(&triangle_sem_conso);
  sem_destroy(&triangle_sem_prod);
  sem_destroy(&fragment_sem_conso);
  sem_destroy(&fragment_sem_prod);
